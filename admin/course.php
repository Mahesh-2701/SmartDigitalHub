    ...
    <script>
        function del(Id){
            if(confirm("If you want to Delete it?")){
                window.location.assign("course.php?Id="+Id);
            }
        }
    </script>
    <?php
...
<body>
    ...
    <section class="my-4">
        <div class="container">
            <form action="course.php" method="POST" enctype="multipart/form-data" class="border rounded p-4 d-flex flex-column gap-2">
                ...
                <input type="text" placeholder="Instructor" class="form-control" name="instructor" required>
    ...
    <?php
          ...
          if(isset($_POST['submit']) && !empty($_FILES['file']['name'])){
            ...
            $instructor=filter_var(trim($_POST['instructor']),FILTER_SANITIZE_SPECIAL_CHARS);
            $price=filter_var(trim($_POST['price']),FILTER_VALIDATE_INT) ;
            $description=htmlspecialchars(trim($_POST['description'])) ;
            $date=$_POST['date'];


            $directory="assets/course/";
            ...
            $format=["jpg","png","JPG","gif","doc","ppt","txt","pdf","mp3","mp4"];
            ...
            if(in_array($fileextensionoffile,$format)){
              ...
              if(in_array($fileextensionoffileimage,$format))
                ...
                $sql="insert into course(title,instructor,image,price,description,released_date,filepath,category_id) 
                                 values('{$title}','{$instructor}','{$fileimagename}',{$price},'{$description}','{$date}','{$filename}',{$categoryid})";
                ...
                echo "<script>alert('Sorry Please Enter Valid Format Video(Only mp4)')</script>";
            }
    ...
    <tr>
        ...
        <td><a href="assets/course/<?php echo $file ?>"><?php echo $file ?></a></td>
        <td><img src="assets/course/<?php echo $image ?>" class="img-fluid" width="100" height="60"><small><?php echo $image ?></small></td>
        ...
        <td>
            ...
            <button class="btn btn-danger" onclick="del(<?php echo $courseid ?>)">Delete</button>
        </td>
    </tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
                ...
                <input type="number" placeholder="Course Id" class="form-control" name="courseid" id="courseid" readonly>
                <label class="form-label">Title</label>
                <input type="text" placeholder="Title" class="form-control" name="title" id="title" required>
                <label class="form-label">Instructor</label>
                <input type="text" placeholder="Instructor" class="form-control" name="instructor" id="instructor" required>
...
<?php
    ...
    if(isset($_POST['update'])){
        ...
        $directorytostore="assets/course/";
        ...
        if(!empty($_FILES['changedfile']['name'])){
            ...
            $allowed=["pdf","doc","docx","ppt","mp3","mp4"];
        ...
        $sql="update course set title='{$title}',instructor='{$instructor}',image='{$currentimage}',price={$price},
                          description='{$description}',released_date='{$date}',filepath='{$currentfile}' where course_id='{$courseid}'";
