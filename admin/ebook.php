    ...
    <script>
        function del(Id){
            if(confirm("If you want to Delete it?")){
                window.location.assign("ebook.php?Id="+Id);
            }
        }
    </script>
    <?php
...
<body>
    ...
    <section class="my-4">
        <div class="container">
            <form action="ebook.php" method="POST" enctype="multipart/form-data" class="border rounded p-4 d-flex flex-column gap-2">
                <h2>Ebook Details</h2>
                <input type="number" placeholder="Category Id" class="form-control" name="categoryid" required>
                <input type="text" placeholder="Book Title" class="form-control" name="title" required>
                <input type="text" placeholder="Book Author" class="form-control" name="author" required>
                ...
                <input type="text" placeholder="Genre" class="form-control" name="genre" required>
          ...
          if(isset($_POST['submit']) && !empty($_FILES['file']['name'])){
            ...
            if(in_array($fileextensionoffile,$format)){
              ...
              if(in_array($fileextensionoffileimage,$format))
                ...
                $sql="insert into ebooks(title,author,image,genre,price,description,released_date,filepath,category_id) 
                                 values('{$title}','{$author}','{$fileimagename}','{$genre}',{$price},'{$description}','{$date}','{$filename}',{$categoryid})";
    ...
    <?php
       ...
       if(isset($_GET['Id'])){
        ...
        $sql="delete from ebooks where ebook_id={$Id}";
    ...
    <tr>
        ...
        <td><?php echo $ebookid ?></td>
        <td><?php echo $title ?></td>
        <td><?php echo $author ?></td>
        <td><a href="assets/ebooks/<?php echo $file ?>"><?php echo $file ?></a></td>
        <td><img src="assets/ebooks/<?php echo $image ?>" class="img-fluid" width="100" height="60"><small><?php echo $image ?></small></td>
        ...
        <td>
            ...
            <button class="btn btn-danger" onclick="del(<?php echo $ebookid ?>)">Delete</button>
        </td>
    </tr>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
                ...
                <label class="form-label">Ebook Id</label>
                <input type="number" placeholder="Id" class="form-control" name="ebookid" id="ebookid" readonly>
                <label class="form-label">Book Title</label>
                <input type="text" placeholder="Book Title" class="form-control" name="title" id="title" required>
                <label class="form-label">Book Author</label>
                <input type="text" placeholder="Book Author" class="form-control" name="author" id="author" required>
                ...
                <input type="text" placeholder="Genre" class="form-control" name="genre" id="genre" required>
...
<?php
    ...
    if(isset($_POST['update'])){
        ...
        $directorytostore="assets/ebooks/";
        ...
        $sql="update ebooks set title='{$title}',author='{$author}',image='{$currentimage}',genre='{$genre}',price={$price},
                          description='{$description}',released_date='{$date}',filepath='{$currentfile}' where ebook_id='{$ebookid}'";
